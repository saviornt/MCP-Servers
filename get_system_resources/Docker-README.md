...
```json
{
  "mcpServers": {
    "get-system-resources": {
      "command": "docker",
      "args": ["run", "--rm", "-i", "--privileged", "--pid=host", "--gpus", "all", "--network=host", "HOST_OS=YOUR_OPERATING_SYSTEM_HERE", "--env", "HOST_RELEASE=YOUR_OS_RELEASE_HERE",
      "davidwadsworth80/get-system-resources:latest"]
    }
  }
}
```
...
```json
{
  "mcpServers": {
    "get-system-resources": {
      "command": "docker",
      "args": ["run", "--rm", "-i", "--privileged", "--pid=host", "--gpus", "all", "--network=host",
      "HOST_OS=YOUR_OPERATING_SYSTEM_HERE", "--env", "HOST_RELEASE=YOUR_OS_RELEASE_HERE",
      "ghcr.io/saviornt/mcp-servers/get-system-resources:latest"]
    }
  }
}
